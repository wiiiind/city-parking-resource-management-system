...
1. `sys_user`
用于保存系统用户基础信息，包含管理员、停车场管理员和车主三类账号，是登录认证、权限识别和个人信息展示的基础表。

2. `sys_role`
用于维护角色编码与角色名称，配合用户角色关联表完成权限划分，便于后续扩展更多系统角色。

3. `sys_user_role`
用于建立用户与角色之间的关联关系，使系统既能支持单角色，也能保留向多角色扩展的能力。

4. `biz_vehicle`
用于保存车主名下车辆档案，记录车牌号、品牌、颜色和所属用户，是停车记录和订单生成的前置基础数据。

5. `biz_parking_lot`
用于保存停车场主数据，包括停车场名称、编号、城市、地址、总车位数、已占用车位数、空闲车位数和营业时间，是停车查询与看板统计的核心表。

6. `biz_parking_region`
用于描述停车场内部分区，例如 A 区、B 区，便于在停车资源管理和单停车场看板中展示更细粒度的数据。

7. `biz_parking_space`
用于保存每个车位的详细信息，包含所属停车场、所属分区、车位编号、车位类型和车位状态，是计算占用率、空闲数以及看板占用车辆情况的核心基础表。

8. `biz_pricing_rule`
用于维护停车场收费规则，包括首段免费时长、每小时费用、封顶金额等，用于车辆出场结算和在场费用计算。

9. `biz_parking_record`
用于保存车辆每次停车过程中的业务记录，包含车辆、停车场、车位、入场时间、出场时间、停车时长、费用和状态，是系统最核心的业务流水表。

10. `biz_order`
用于保存停车缴费订单，记录关联停车记录、订单金额、支付状态和支付时间，用于车主端订单查看和管理员后台订单统计。

11. `sys_notice`
用于保存系统公告内容，可在首页或系统模块中向用户展示演示账号说明、运营提醒等信息。

12. `sys_operation_log`
用于保存关键操作日志，例如车辆入场、车辆出场、订单支付、停车场新增等，便于答辩展示系统可追溯性。
...
- 后端采用 MySQL 8 作为真实业务数据库，业务逻辑通过 `JdbcTemplate` 直接访问数据库表完成读写操作。
- 项目提供 `bootstrap.sql`、`schema.sql` 和 `data.sql`，可用于一键建库、建表和导入演示数据。
